...
`),e=[],n=r[0].toLowerCase(),i=n.includes("date")||n.includes("isin")?1:0;for(let s=i;s<r.length;s++){let l=r[s].split(",").map(c=>c.trim());if(l.length>=5){let[c,u,d,E,N]=l;c&&u&&d&&E&&N&&e.push({date:c,isin:u.toUpperCase(),quantity:parseFloat(d),price:parseFloat(E),type:N.toUpperCase()})}else if(l.length>=4){let[c,u,d,E]=l;c&&u&&d&&E&&e.push({date:c,isin:u.toUpperCase(),quantity:parseFloat(d),price:parseFloat(E),type:"BUY"})}}return e}var m=`
    ...
    )`,M="DROP TABLE IF EXISTS transactions",I=`
...
ORDER BY sell_date, isin, buy_price`}var at=`
    ...
    FROM remaining_lots
    WHERE shares > 0`,x=[at,lt,ct,ut];function _(t,r="t.sell_date"){if(!t)return"";let e=[];return t.isin&&e.push(`t.isin = '${t.isin}'`),t.sector&&e.push(`s.sector = '${t.sector}'`),t.dateFrom&&e.push(`${r} >= '${t.dateFrom}'`),t.dateTo&&e.push(`${r} <= '${t.dateTo}'`),e.length>0?"WHERE "+e.join(" AND "):""}function v(t){return`
    SELECT
        t.isin,
        s.name,
        s.sector,
...
- Use INR (\u20B9) for currency values.`,ft=8;async function J(t,r,e){let n=r.apiKey||L,o=r.model?[r.model]:r.apiKey?[St]:_t;if(!n||n.startsWith("__OPENROUTER_"))throw new Error("No API key configured. Please enter your OpenRouter API key.");let i=[{role:"system",content:gt},{role:"user",content:"Analyze my portfolio and provide key insights."}];for(let s=0;s<ft;s++){e?.(`Thinking... (step ${s+1})`);let l=await bt(n,o,i);if(!l.tool_calls||l.tool_calls.length===0)return l.content||"No insights generated.";i.push({role:"assistant",content:l.content,tool_calls:l.tool_calls});for(let c of l.tool_calls){e?.(`Querying: ${c.function.name}...`);let u;try{let d=JSON.parse(c.function.arguments);u=await Et(t,c.function.name,d)}catch(d){u=JSON.stringify({error:String(d)})}i.push({role:"tool",tool_call_id:c.id,content:u})}}return"Analysis incomplete \u2014 reached maximum exploration steps."}async function bt(t,r,e){let n=null;for(let o of r)try{return await Rt(t,o,e)}catch(i){if(n=i instanceof Error?i:new Error(String(i)),!/429|404|403/.test(n.message))throw n}throw n||new Error("All models failed")}async function Rt(t,r,e){for(let o=0;o<3;o++){let i=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${t}`,"Content-Type":"application/json"},body:JSON.stringify({model:r,messages:e,tools:yt,max_tokens:4096})});if(i.status===429&&o<2){await new Promise(c=>setTimeout(c,3e3*(o+1)));continue}if(!i.ok){let c=await i.text();throw new Error(`API error ${i.status}: ${c}`)}let l=(await i.json()).choices?.[0]?.message;if(!l)throw new Error("No response from LLM");return{content:l.content??null,tool_calls:l.tool_calls}}throw new Error("Max retries exceeded")}var K={dataPath:"data",benchmarks:{nifty50:"nifty50.parquet",bank_nifty:"bank_nifty.parquet",sensex:"sensex.parquet",nifty_midcap:"nifty_midcap.parquet",bse_500:"bse_500.parquet"}},D=null,h=null,a={csvInput:document.getElementById("csvInput"),benchmarkSelect:document.getElementById("benchmarkSelect"),initialCapital:document.getElementById("initialCapital"),loadingSection:document.getElementById("loadingSection"),loadingText:document.getElementById("loadingText"),errorSection:document.getElementById("errorSection"),errorMessage:document.getElementById("errorMessage"),retryBtn:document.getElementById("retryBtn"),insightsSection:document.getElementById("insightsSection"),apiKey:document.getElementById("apiKey"),generateInsightsBtn:document.getElementById("generateInsights"),insightsStatus:document.getElementById("insightsStatus"),insightsOutput:document.getElementById("insightsOutput")};async function w(){O("Initializing DuckDB...");try{await B(),y()}catch(t){y(),R("Failed to initialize DuckDB: "+(t instanceof Error?t.message:String(t)))}}function ht(){a.csvInput.addEventListener("change",X),a.retryBtn.addEventListener("click",()=>{a.errorSection.style.display="none",w()}),a.benchmarkSelect.addEventListener("change",async()=>{h&&await Z()}),a.apiKey.addEventListener("input",()=>{a.generateInsightsBtn.disabled=!a.apiKey.value.trim()}),a.generateInsightsBtn.addEventListener("click",Lt)}async function X(t){let e=t.target.files?.[0];if(e){O("Parsing transactions...");try{let n=await e.text(),o=C(n);if(o.length===0)throw new Error("No valid transactions found in CSV");if(o.sort((i,s)=>new Date(i.date).getTime()-new Date(s.date).getTime()),!k())throw new Error("Database not initialized. Please refresh the page and try again.");h=o,await V(b(),o),await Z(),y()}catch(n){y(),R("Error processing CSV: "+(n instanceof Error?n.message:String(n)))}}}async function Z(){if(h){O("Analyzing portfolio...");try{let t=parseFloat(a.initialCapital.value)||1e5,r=b(),e=await z(r,t);if(!e){R("No portfolio data available for analysis"),y();return}let n=await j(r,t),o=await At(a.benchmarkSelect.value);if(Tt(e,n,o),a.insightsSection.style.display="block",y(),Q())tt();else{let i=document.querySelector(".insights-upgrade");i&&(i.open=!0)}}catch(t){y(),R("Analysis error: "+(t instanceof Error?t.message:String(t)))}}}async function At(t){let r=K.benchmarks[t];try{return(await T(`
            SELECT
                date,
                close_price,
