var F,C=new Map;function At(t){return t.replace(/\//g,"__").replace(/[^A-Za-z0-9._-]/g,"_")}function bt(){return F||(F=(async()=>{try{let n=await(await navigator.storage.getDirectory()).getDirectoryHandle("parquet-cache",{create:!0}),i=await(await n.getFileHandle("__probe__",{create:!0})).createWritable();return await i.write(new Uint8Array([0])),await i.close(),await n.removeEntry("__probe__"),n}catch{return null}})()),F}async function Tt(t,n){try{let i=await(await t.getFileHandle(n+".meta")).getFile();return JSON.parse(await i.text())}catch{return null}}async function V(t,n){try{return await(await(await t.getFileHandle(n)).getFile()).arrayBuffer()}catch{return null}}function G(t,n,e,i){(async()=>{try{let s=await(await t.getFileHandle(n,{create:!0})).createWritable();await s.write(e),await s.close();let o=await(await t.getFileHandle(n+".meta",{create:!0})).createWritable();await o.write(JSON.stringify(i)),await o.close()}catch(r){console.warn("[opfs-cache] write failed:",r)}})()}async function Rt(t,n){await Promise.all([t.removeEntry(n).catch(()=>{}),t.removeEntry(n+".meta").catch(()=>{})])}async function z(t,n){let e=At(t),i=C.get(e);if(i)return i;let r=wt(n,e);return C.set(e,r),r.then(()=>C.delete(e),()=>C.delete(e)),r}async function wt(t,n){let e=await bt();if(!e){let a=await fetch(t);if(!a.ok)throw new Error(`Failed to fetch ${t}: ${a.status}`);return a.arrayBuffer()}let i=await Tt(e,n);if(i){let a={};if(i.etag?a["If-None-Match"]=i.etag:i.lastModified&&(a["If-Modified-Since"]=i.lastModified),Object.keys(a).length>0){let o=null,l=null;try{o=await fetch(t,{headers:a})}catch(u){l=u}if(l||o&&o.status>=500){let u=await V(e,n);if(u)return u;throw l||new Error(`Failed to fetch ${t}: ${o.status}`)}if(o.status>=400)throw await Rt(e,n),new Error(`Failed to fetch ${t}: ${o.status}`);if(o.status===304){let u=await V(e,n);if(u)return u}else{let u=await o.arrayBuffer();return G(e,n,u,{etag:o.headers.get("etag")??void 0,lastModified:o.headers.get("last-modified")??void 0}),u}}}let r=await fetch(t);if(!r.ok)throw new Error(`Failed to fetch ${t}: ${r.status}`);let s=await r.arrayBuffer();return G(e,n,s,{etag:r.headers.get("etag")??void 0,lastModified:r.headers.get("last-modified")??void 0}),s}var Ct="https://cdn.jsdelivr.net/npm/@duckdb/duckdb-wasm@1.28.0/+esm",L=null,_=null,y=null;async function Lt(){return L||(L=await import(Ct),L)}async function j(){if(y)return;let t=await Lt(),n=t.getJsDelivrBundles(),e=await t.selectBundle(n),i=URL.createObjectURL(new Blob([`importScripts("${e.mainWorker}");`],{type:"text/javascript"})),r=new Worker(i),s=new t.ConsoleLogger;_=new t.AsyncDuckDB(s,r),await _.instantiate(e.mainModule,e.pthreadWorker),y=await _.connect()}function J(){return y!==null}async function Dt(t){if(!y)throw new Error("DuckDB not initialized. Call initDuckDB() first.");await y.query(t)}async function P(t){if(!y)throw new Error("DuckDB not initialized. Call initDuckDB() first.");let n=await y.query(t);return Ot(n)}async function D(t,n){if(!_)throw new Error("DuckDB not initialized. Call initDuckDB() first.");let e=await z(t,n);await _.registerFileBuffer(t,new Uint8Array(e))}function O(){return{exec:Dt,query:P}}function Ot(t){let n=[],e=t.schema.fields,i=t.numRows;for(let r=0;r<i;r++){let s={};for(let a of e){let o=t.getChild(a.name);s[a.name]=Nt(o.get(r))}n.push(s)}return n}function Nt(t){if(t==null)return t;if(typeof t=="bigint")return Number(t);if(typeof t=="object"&&t!==null&&!(t instanceof Date)){let n=t;if(typeof n.toString=="function"){let e=n.toString();if(e!==""&&e!=="[object Object]"&&!isNaN(Number(e)))return Number(e)}}return typeof t=="string"&&t!==""&&!isNaN(Number(t))?Number(t):t}var It={name:"Groww",delimiter:"	",headerSignature:["isin","execution date"],columns:{isin:"isin",type:"type",quantity:"quantity",value:"value",date:"execution date",status:"status"},acceptStatus:["executed"]},Mt={name:"Generic CSV",delimiter:",",headerSignature:["isin"],columns:{isin:"isin",type:"type",quantity:"quantity",price:"price",value:"value",date:"date",status:"status"},acceptStatus:["executed"]},Bt=[It,Mt];function x(t,n){return t.split(n).map(e=>e.trim())}function kt(t){let n=t.match(/^(\d{2})-(\d{2})-(\d{4})/);return n?`${n[3]}-${n[2]}-${n[1]}`:/^\d{4}-\d{2}-\d{2}/.test(t)?t.slice(0,10):null}function Ft(t){for(let n=0;n<t.length;n++){let e=t[n].toLowerCase();if(!e.includes("isin"))continue;let i=(t[n].match(/\t/g)??[]).length>=2;for(let r of Bt){if(!(r.delimiter==="	"?i:!i)||!r.headerSignature.every(l=>e.includes(l)))continue;let a=x(t[n],r.delimiter).map(l=>l.toLowerCase()),o=l=>a.some(u=>u.includes(l));if(!(!o(r.columns.isin)||!o(r.columns.quantity)||!o(r.columns.date)))return{config:r,headerIdx:n}}}return null}function Pt(t,n,e){let i=[],r=x(t[e],n.delimiter).map(m=>m.toLowerCase()),s=m=>r.findIndex(R=>R.includes(m)),{columns:a}=n,o=s(a.isin),l=s(a.quantity),u=s(a.date),d=a.type!=null?s(a.type):-1,g=a.price!=null?s(a.price):-1,$=a.value!=null?s(a.value):-1,B=a.status!=null?s(a.status):-1;if(o<0||l<0||u<0)return i;for(let m=e+1;m<t.length;m++){let R=t[m].trim();if(!R)continue;let p=x(R,n.delimiter);if(p.length<3||B>=0&&n.acceptStatus&&p[B]&&!n.acceptStatus.includes(p[B].toLowerCase()))continue;let W=(p[o]??"").toUpperCase();if(!W)continue;let w=parseFloat(p[l]??"");if(!w||w<=0)continue;let Y=kt(p[u]??"");if(!Y)continue;let ht=(d>=0?(p[d]??"").toUpperCase():"")==="SELL"?"SELL":n.defaultType??"BUY",S;if($>=0&&g<0){let k=parseFloat(p[$]??"");if(!k||k<=0)continue;S=k/w}else if(g>=0){if(S=parseFloat(p[g]??""),!S||S<=0)continue}else continue;i.push({date:Y,isin:W,quantity:w,price:S,type:ht})}return i}function xt(t){let n=[],e=t[0]?.toLowerCase()??"",r=e.includes("date")||e.includes("symbol")?1:0;for(let s=r;s<t.length;s++){let a=t[s].split(",").map(o=>o.trim());if(a.length>=5){let[o,l,u,d,g]=a;o&&l&&u&&d&&g&&n.push({date:o,isin:l.toUpperCase(),quantity:parseFloat(u),price:parseFloat(d),type:g.toUpperCase()})}else if(a.length>=4){let[o,l,u,d]=a;o&&l&&u&&d&&n.push({date:o,isin:l.toUpperCase(),quantity:parseFloat(u),price:parseFloat(d),type:"BUY"})}}return n}var vt=["name","unique client code","client code","client id","pan"];function Q(t){return t.split(`
`).map(n=>{let e=n.indexOf("	");if(e<0)return n;let i=n.slice(0,e).trim().toLowerCase();return vt.some(r=>i===r)?n.slice(0,e+1)+"[REDACTED]":n}).join(`
...
`),e=Ft(n);return e?Pt(n,e.config,e.headerIdx):xt(n)}var f=`
    ...
    )`,K="DROP TABLE IF EXISTS transactions",X=`
...
FROM raw`}function tt(){return`
WITH ${f}
SELECT isin, cost_basis AS cost_basis, shares
FROM remaining_lots
WHERE shares > 0
ORDER BY isin, cost_basis`}function et(){return`
WITH ${f}
SELECT
...
FROM fifo_matches
ORDER BY sell_date, isin, buy_price`}var qt=`
    ...
    FROM remaining_lots
    WHERE shares > 0`,nt=[qt,Ht,$t,Wt];function h(t,n="t.sell_date"){if(!t)return"";let e=[];return t.isin&&e.push(`t.isin = '${t.isin}'`),t.sector&&e.push(`s.sector = '${t.sector}'`),t.dateFrom&&e.push(`${n} >= '${t.dateFrom}'`),t.dateTo&&e.push(`${n} <= '${t.dateTo}'`),e.length>0?"WHERE "+e.join(" AND "):""}function it(t){return`
    SELECT
        t.isin,
        s.name,
        s.sector,
    ...
    ORDER BY total_pnl DESC`}function rt(t,n){let e={week:"c.year_week",month:"c.year_month",quarter:"c.year_quarter",year:"c.year"}[t];return`
    SELECT
        ${e}                AS period,
        COUNT(*)              AS num_trades,
        SUM(t.pnl)           AS total_pnl,
    ...
    ${h(n)}
    GROUP BY ${e}
    ORDER BY ${e}`}function st(t,n,e=5,i){let r={week:"c.year_week",month:"c.year_month",quarter:"c.year_quarter",year:"c.year"}[t],s=n==="best"?"DESC":"ASC";return`
    SELECT
        ${r}                AS period,
        COUNT(*)              AS num_trades,
        SUM(t.pnl)           AS total_pnl
    ...
    ORDER BY value DESC`}function ot(t){return`
    SELECT
    ...
    ORDER BY total_pnl DESC`}function ct(t){return`
    SELECT
        t.trade_id,
        t.isin,
        s.name,
    ...
    LEFT JOIN dim_stock s ON t.isin = s.isin
    ${h(t)}
    ORDER BY t.sell_date DESC, t.pnl DESC`}async function lt(t,n){if(await t.exec(K),await t.exec(X),n.length===0)return;let e=1e3;for(let i=0;i<n.length;i+=e){let s=n.slice(i,i+e).map((a,o)=>`(${i+o}, '${a.date}', '${a.isin}', ${a.quantity}, ${a.price}, '${a.type}')`).join(", ");await t.exec(`INSERT INTO transactions VALUES ${s}`)}await Yt(t)}async function Yt(t){for(let n of nt)await t.exec(n)}async function ut(t,n){let e=await t.query(Z(n));if(e.length===0||e[0].first_date==null)return null;let i=e[0],r=await Vt(t),s=await Gt(t);return{totalInvested:Number(i.total_invested),totalProceeds:Number(i.total_proceeds),realizedPnL:Number(i.realized_pnl),costBasisRemaining:Number(i.cost_basis_remaining),cashBalance:Number(i.cash_balance),portfolioValue:Number(i.portfolio_value),totalReturn:Number(i.total_return),totalReturnPct:Number(i.total_return_pct),numStocks:Number(i.num_stocks),holdingDays:Number(i.holding_days),holdingYears:Number(i.holding_years),annualizedReturnPct:Number(i.annualized_return_pct),firstDate:String(i.first_date),lastDate:String(i.last_date),holdings:r,sellDetails:s}}async function Vt(t){return(await t.query(tt())).map(e=>({isin:String(e.isin),shares:Number(e.shares),costBasis:Number(e.cost_basis)}))}async function Gt(t){return(await t.query(et())).map(e=>({date:String(e.date),isin:String(e.isin),sharesSold:Number(e.shares_sold),salePrice:Number(e.sale_price),costBasis:Number(e.cost_basis),pnl:Number(e.pnl)}))}var A={isin:{type:"string",description:"Filter by ISIN (e.g. INE002A01018)"},sector:{type:"string",description:"Filter by sector"},dateFrom:{type:"string",description:"Start date (YYYY-MM-DD)"},dateTo:{type:"string",description:"End date (YYYY-MM-DD)"}},zt=[{type:"function",function:{name:"pnl_by_stock",description:"Get realized PnL breakdown by stock. Shows total PnL, return %, trade count, and avg holding days per ISIN.",parameters:{type:"object",properties:A,required:[]}}},{type:"function",function:{name:"pnl_by_period",description:"Get realized PnL aggregated by time period. Choose granularity: week, month, quarter, or year.",parameters:{type:"object",properties:{granularity:{type:"string",enum:["week","month","quarter","year"],description:"Time granularity"},...A},required:["granularity"]}}},{type:"function",function:{name:"top_periods",description:"Get the best or worst performing periods by PnL.",parameters:{type:"object",properties:{granularity:{type:"string",enum:["week","month","quarter","year"]},direction:{type:"string",enum:["best","worst"]},limit:{type:"number",description:"Number of periods to return (default 5)"},...A},required:["granularity","direction"]}}},{type:"function",function:{name:"holdings_concentration",description:"Get current portfolio holdings with concentration weights (% of total value).",parameters:{type:"object",properties:{},required:[]}}},{type:"function",function:{name:"pnl_by_sector",description:"Get realized PnL aggregated by sector.",parameters:{type:"object",properties:A,required:[]}}},{type:"function",function:{name:"trade_detail",description:"Get individual FIFO-matched trade details with buy/sell dates, prices, PnL, and holding days.",parameters:{type:"object",properties:A,required:[]}}},{type:"function",function:{name:"run_sql",description:"Run arbitrary read-only SQL against the portfolio database. Available tables: transactions (seq, date, isin, quantity, price, type), fact_trades (trade_id, isin, buy_date, sell_date, buy_price, sell_price, quantity, cost_basis, proceeds, pnl, return_pct, holding_days), fact_positions (isin, buy_date, cost_basis, quantity, value_at_cost), dim_calendar (date, year, quarter, month, week, year_month, year_week, year_quarter), dim_stock (isin, name, sector, industry).",parameters:{type:"object",properties:{sql:{type:"string",description:"SQL SELECT query to execute"}},required:["sql"]}}}];function b(t){let n={};return typeof t.isin=="string"&&(n.isin=t.isin),typeof t.sector=="string"&&(n.sector=t.sector),typeof t.dateFrom=="string"&&(n.dateFrom=t.dateFrom),typeof t.dateTo=="string"&&(n.dateTo=t.dateTo),Object.keys(n).length>0?n:void 0}async function jt(t,n,e){let i;switch(n){case"pnl_by_stock":i=it(b(e));break;case"pnl_by_period":i=rt(e.granularity,b(e));break;case"top_periods":i=st(e.granularity,e.direction,typeof e.limit=="number"?e.limit:5,b(e));break;case"holdings_concentration":i=at();break;case"pnl_by_sector":i=ot(b(e));break;case"trade_detail":i=ct(b(e));break;case"run_sql":{let a=String(e.sql).trim();if(!a.toUpperCase().startsWith("SELECT"))return JSON.stringify({error:"Only SELECT queries are allowed"});i=a;break}default:return JSON.stringify({error:`Unknown tool: ${n}`})}let r=await t.query(i),s=JSON.stringify(r);if(s.length>8e3){let a=r.slice(0,20);return JSON.stringify({rows:a,note:`Showing 20 of ${r.length} rows. Use filters to narrow results.`})}return s}var U="__OPENROUTER_API_KEY__",Jt=["stepfun/step-3.5-flash:free","meta-llama/llama-3.3-70b-instruct:free"],Qt="anthropic/claude-sonnet-4";function dt(){return U.length>0&&!U.startsWith("__OPENROUTER_")}var Kt=`You are an expert portfolio analyst. Analyze the user's stock trading data using the available tools.
...
- Use \u20B9 for currency. Keep each bullet to 1-2 sentences max.`,Xt=8;async function pt(t,n,e){let i=n.apiKey||U,r=n.model?[n.model]:n.apiKey?[Qt]:Jt;if(!i||i.startsWith("__OPENROUTER_"))throw new Error("No API key configured. Please enter your OpenRouter API key.");let s=[{role:"system",content:Kt},{role:"user",content:"Analyze my portfolio and provide key insights."}];for(let a=0;a<Xt;a++){e?.(`Thinking... (step ${a+1})`);let o=await Zt(i,r,s);if(!o.tool_calls||o.tool_calls.length===0)return o.content||"No insights generated.";s.push({role:"assistant",content:o.content,tool_calls:o.tool_calls});for(let l of o.tool_calls){e?.(`Querying: ${l.function.name}...`);let u;try{let d=JSON.parse(l.function.arguments);u=await jt(t,l.function.name,d)}catch(d){u=JSON.stringify({error:String(d)})}s.push({role:"tool",tool_call_id:l.id,content:u})}}return"Analysis incomplete \u2014 reached maximum exploration steps."}async function Zt(t,n,e){let i=null;for(let r of n)try{return await te(t,r,e)}catch(s){if(i=s instanceof Error?s:new Error(String(s)),!/429|404|403/.test(i.message))throw i}throw i||new Error("All models failed")}async function te(t,n,e){for(let r=0;r<3;r++){let s=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${t}`,"Content-Type":"application/json"},body:JSON.stringify({model:n,messages:e,tools:zt,max_tokens:1024})});if(s.status===429&&r<2){await new Promise(l=>setTimeout(l,3e3*(r+1)));continue}if(!s.ok){let l=await s.text();throw new Error(`API error ${s.status}: ${l}`)}let o=(await s.json()).choices?.[0]?.message;if(!o)throw new Error("No response from LLM");return{content:o.content??null,tool_calls:o.tool_calls}}throw new Error("Max retries exceeded")}var N=null;async function ee(){if(N)return N;let t=await fetch("data/isin_map.json");if(!t.ok)throw new Error("Failed to load ISIN map");return N=await t.json(),N}async function ne(t,n){await t.exec("DROP TABLE IF EXISTS stock_prices"),await t.exec("CREATE TABLE stock_prices (date DATE, symbol VARCHAR, isin VARCHAR, close DOUBLE)");let e=await t.query("SELECT DISTINCT isin FROM transactions");for(let i of e){let r=String(i.isin),s=n[r];if(!s)continue;let a=`${s}.parquet`;try{await D(`stock_prices/${a}`,`data/stock_prices/${a}`),await t.exec(`
...
ORDER BY ca.date
`;async function mt(t,n){let e=await ee();return await ne(t,e),(await t.query(ie(n))).map(r=>{let s=r.date,a;return typeof s=="number"?a=s:s instanceof Date?a=s.getTime():a=new Date(String(s)).getTime(),{date:a,value:Number(r.portfolio_value)||0,cash:Number(r.cash)||0,returnPct:Number(r.return_pct)||0}})}var yt={dataPath:"data",benchmarks:{nifty50:"nifty50.parquet",bank_nifty:"bank_nifty.parquet",sensex:"sensex.parquet",nifty_midcap:"nifty_midcap.parquet",bse_500:"bse_500.parquet"}},q=null,M=null,Et=1e5,c={csvInput:document.getElementById("csvInput"),benchmarkSelect:document.getElementById("benchmarkSelect"),loadingSection:document.getElementById("loadingSection"),loadingText:document.getElementById("loadingText"),errorSection:document.getElementById("errorSection"),errorMessage:document.getElementById("errorMessage"),retryBtn:document.getElementById("retryBtn"),uploadSection:document.getElementById("uploadSection"),changeFileBtn:document.getElementById("changeFile"),insightsSection:document.getElementById("insightsSection"),insightsSummary:document.getElementById("insightsSummary"),insightsFull:document.getElementById("insightsFull"),insightsToggle:document.getElementById("insightsToggle"),apiKey:document.getElementById("apiKey"),generateInsightsBtn:document.getElementById("generateInsights"),insightsStatus:document.getElementById("insightsStatus")};async function H(){T("Initializing DuckDB...");try{await j(),E()}catch(t){E(),I("Failed to initialize DuckDB: "+(t instanceof Error?t.message:String(t)))}}function re(){c.csvInput.addEventListener("change",ft),c.retryBtn.addEventListener("click",()=>{c.errorSection.style.display="none",H()}),c.benchmarkSelect.addEventListener("change",async()=>{M&&await St()}),c.apiKey.addEventListener("input",()=>{c.generateInsightsBtn.disabled=!c.apiKey.value.trim()}),c.generateInsightsBtn.addEventListener("click",ce),c.insightsToggle.addEventListener("click",()=>{let t=c.insightsFull.style.display!=="none";c.insightsFull.style.display=t?"none":"block",c.insightsToggle.textContent=t?"Show more":"Show less"}),c.changeFileBtn.addEventListener("click",()=>{c.insightsSection.style.display="none",c.uploadSection.style.display="block",c.csvInput.value=""})}async function ft(t){let e=t.target.files?.[0];if(e){T("Parsing transactions...");try{let i=Q(await e.text()),r=v(i);if(r.length===0)throw new Error("No valid transactions found in CSV");if(r.sort((s,a)=>new Date(s.date).getTime()-new Date(a.date).getTime()),!J())throw new Error("Database not initialized. Please refresh the page and try again.");if(M=r,await lt(O(),r),await St(),E(),dt())_t();else{let s=document.querySelector(".insights-upgrade");s&&(s.open=!0)}}catch(i){E(),I("Error processing CSV: "+(i instanceof Error?i.message:String(i)))}}}async function St(){if(M){T("Analyzing portfolio...");try{let t=O(),n=await ut(t,Et);if(!n){I("No portfolio data available for analysis"),E();return}T("Loading stock prices...");let e=await mt(t,Et);T("Loading benchmark...");let i=await se(c.benchmarkSelect.value,e.length>0?e[0].date:0,e.length>0?e[e.length-1].date:0);ae(n,e,i),c.uploadSection.style.display="none",c.insightsSection.style.display="block",E()}catch(t){E(),I("Analysis error: "+(t instanceof Error?t.message:String(t)))}}}async function se(t,n,e){if(!n||!e)return[];let i=yt.benchmarks[t];try{await D(i,`${yt.dataPath}/${i}`);let r=new Date(n).toISOString().slice(0,10),s=new Date(e).toISOString().slice(0,10);return(await P(`
            WITH base AS (
                SELECT date, close
                FROM parquet_scan('${i}')
