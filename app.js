var et="https://cdn.jsdelivr.net/npm/@duckdb/duckdb-wasm@1.28.0/+esm",f=null,A=null,p=null;async function nt(){return f||(f=await import(et),f)}async function N(){if(p)return;let t=await nt(),r=t.getJsDelivrBundles(),e=await t.selectBundle(r),n=URL.createObjectURL(new Blob([`importScripts("${e.mainWorker}");`],{type:"text/javascript"})),a=new Worker(n),i=new t.ConsoleLogger;A=new t.AsyncDuckDB(i,a),await A.instantiate(e.mainModule,e.pthreadWorker),p=await A.connect()}function k(){return p!==null}async function rt(t){if(!p)throw new Error("DuckDB not initialized. Call initDuckDB() first.");await p.query(t)}async function T(t){if(!p)throw new Error("DuckDB not initialized. Call initDuckDB() first.");let r=await p.query(t);return it(r)}function b(){return{exec:rt,query:T}}function it(t){let r=[],e=t.schema.fields,n=t.numRows;for(let a=0;a<n;a++){let i={};for(let o of e){let l=t.getChild(o.name);i[o.name]=st(l.get(a))}r.push(i)}return r}function st(t){if(t==null)return t;if(typeof t=="bigint")return Number(t);if(typeof t=="object"&&t!==null&&!(t instanceof Date)){let r=t;if(typeof r.toString=="function"){let e=r.toString();if(e!==""&&e!=="[object Object]"&&!isNaN(Number(e)))return Number(e)}}return typeof t=="string"&&t!==""&&!isNaN(Number(t))?Number(t):t}function C(t){let r=t.trim().split(`
`),e=[],n=r[0].toLowerCase(),i=n.includes("date")||n.includes("isin")?1:0;for(let o=i;o<r.length;o++){let l=r[o].split(",").map(c=>c.trim());if(l.length>=5){let[c,u,d,E,B]=l;c&&u&&d&&E&&B&&e.push({date:c,isin:u.toUpperCase(),quantity:parseFloat(d),price:parseFloat(E),type:B.toUpperCase()})}else if(l.length>=4){let[c,u,d,E]=l;c&&u&&d&&E&&e.push({date:c,isin:u.toUpperCase(),quantity:parseFloat(d),price:parseFloat(E),type:"BUY"})}}return e}var m=`
...
ORDER BY sell_date, isin, buy_price`}var at=`
    ...
    ${_(r)}
    GROUP BY ${e}
    ORDER BY ${e}`}function $(t,r,e=5,n){let a={week:"c.year_week",month:"c.year_month",quarter:"c.year_quarter",year:"c.year"}[t],i=r==="best"?"DESC":"ASC";return`
    SELECT
        ${a}                AS period,
        COUNT(*)              AS num_trades,
        SUM(t.pnl)           AS total_pnl
...
- Use INR (\u20B9) for currency values.`,ft=8;async function J(t,r,e){let n=r.apiKey||L,a=r.model?[r.model]:r.apiKey?[St]:_t;if(!n||n.startsWith("__OPENROUTER_"))throw new Error("No API key configured. Please enter your OpenRouter API key.");let i=[{role:"system",content:gt},{role:"user",content:"Analyze my portfolio and provide key insights."}];for(let o=0;o<ft;o++){e?.(`Thinking... (step ${o+1})`);let l=await bt(n,a,i);if(!l.tool_calls||l.tool_calls.length===0)return l.content||"No insights generated.";i.push({role:"assistant",content:l.content,tool_calls:l.tool_calls});for(let c of l.tool_calls){e?.(`Querying: ${c.function.name}...`);let u;try{let d=JSON.parse(c.function.arguments);u=await Et(t,c.function.name,d)}catch(d){u=JSON.stringify({error:String(d)})}i.push({role:"tool",tool_call_id:c.id,content:u})}}return"Analysis incomplete \u2014 reached maximum exploration steps."}async function bt(t,r,e){let n=null;for(let a of r)try{return await Rt(t,a,e)}catch(i){if(n=i instanceof Error?i:new Error(String(i)),!/429|404|403/.test(n.message))throw n}throw n||new Error("All models failed")}async function Rt(t,r,e){for(let a=0;a<3;a++){let i=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${t}`,"Content-Type":"application/json"},body:JSON.stringify({model:r,messages:e,tools:yt,max_tokens:4096})});if(i.status===429&&a<2){await new Promise(c=>setTimeout(c,3e3*(a+1)));continue}if(!i.ok){let c=await i.text();throw new Error(`API error ${i.status}: ${c}`)}let l=(await i.json()).choices?.[0]?.message;if(!l)throw new Error("No response from LLM");return{content:l.content??null,tool_calls:l.tool_calls}}throw new Error("Max retries exceeded")}var K={dataPath:"data",benchmarks:{nifty50:"nifty50.parquet",bank_nifty:"bank_nifty.parquet",sensex:"sensex.parquet",nifty_midcap:"nifty_midcap.parquet",bse_500:"bse_500.parquet"}},D=null,h=null,s={csvInput:document.getElementById("csvInput"),benchmarkSelect:document.getElementById("benchmarkSelect"),initialCapital:document.getElementById("initialCapital"),loadingSection:document.getElementById("loadingSection"),loadingText:document.getElementById("loadingText"),errorSection:document.getElementById("errorSection"),errorMessage:document.getElementById("errorMessage"),retryBtn:document.getElementById("retryBtn"),uploadSection:document.getElementById("uploadSection"),changeFileBtn:document.getElementById("changeFile"),insightsSection:document.getElementById("insightsSection"),apiKey:document.getElementById("apiKey"),generateInsightsBtn:document.getElementById("generateInsights"),insightsStatus:document.getElementById("insightsStatus"),insightsOutput:document.getElementById("insightsOutput")};async function w(){O("Initializing DuckDB...");try{await N(),y()}catch(t){y(),R("Failed to initialize DuckDB: "+(t instanceof Error?t.message:String(t)))}}function ht(){s.csvInput.addEventListener("change",X),s.retryBtn.addEventListener("click",()=>{s.errorSection.style.display="none",w()}),s.benchmarkSelect.addEventListener("change",async()=>{h&&await Z()}),s.apiKey.addEventListener("input",()=>{s.generateInsightsBtn.disabled=!s.apiKey.value.trim()}),s.generateInsightsBtn.addEventListener("click",Lt),s.changeFileBtn.addEventListener("click",()=>{s.insightsSection.style.display="none",s.uploadSection.style.display="block",s.csvInput.value=""})}async function X(t){let e=t.target.files?.[0];if(e){O("Parsing transactions...");try{let n=await e.text(),a=C(n);if(a.length===0)throw new Error("No valid transactions found in CSV");if(a.sort((i,o)=>new Date(i.date).getTime()-new Date(o.date).getTime()),!k())throw new Error("Database not initialized. Please refresh the page and try again.");h=a,await V(b(),a),await Z(),y()}catch(n){y(),R("Error processing CSV: "+(n instanceof Error?n.message:String(n)))}}}async function Z(){if(h){O("Analyzing portfolio...");try{let t=parseFloat(s.initialCapital.value)||1e5,r=b(),e=await z(r,t);if(!e){R("No portfolio data available for analysis"),y();return}let n=await j(r,t),a=await At(s.benchmarkSelect.value);if(Tt(e,n,a),s.uploadSection.style.display="none",s.insightsSection.style.display="block",y(),Q())tt();else{let i=document.querySelector(".insights-upgrade");i&&(i.open=!0)}}catch(t){y(),R("Analysis error: "+(t instanceof Error?t.message:String(t)))}}}async function At(t){let r=K.benchmarks[t];try{return(await T(`
            SELECT
                date,
                close_price,
            ...
            ORDER BY date
        `)).map(n=>({date:n.date,closePrice:Number(n.close_price)||0,cumulativeReturnPct:Number(n.cumulative_return_pct)||0}))}catch(e){return console.warn(`Could not load benchmark data for ${t}:`,e.message),[]}}function Tt(t,r,e){let n=l=>document.getElementById(l),a=n("totalReturn");a.textContent=`${t.totalReturn>=0?"+":""}${t.totalReturnPct.toFixed(2)}%`,a.className="stat-value "+(t.totalReturn>=0?"positive":"negative");let i=n("annualizedReturn");i.textContent=`${t.annualizedReturnPct>=0?"+":""}${t.annualizedReturnPct.toFixed(2)}%`,i.className="stat-value "+(t.annualizedReturnPct>=0?"positive":"negative"),n("portfolioValue").textContent=`\u20B9${t.portfolioValue.toLocaleString("en-IN")}`,n("holdingPeriod").textContent=`${t.holdingDays} days (${t.holdingYears.toFixed(1)} years)`;let o=n("pnlGrid");if(t.realizedPnL!==0){o.style.display="grid";let l=n("totalRealizedPnL");l.textContent=`${t.realizedPnL>=0?"+":""}\u20B9${t.realizedPnL.toLocaleString("en-IN")}`,l.className="stat-value "+(t.realizedPnL>=0?"positive":"negative"),n("totalUnrealizedGain").textContent="-",n("fifoCostBasis").textContent=`\u20B9${t.costBasisRemaining.toLocaleString("en-IN")}`,n("stocksCount").textContent=`${t.numStocks}`}else o.style.display="none";Ct(r,e)}function Ct(t,r){let e=document.getElementById("returnsChart").getContext("2d"),n=t.map(o=>o.date),a=t.map(o=>o.returnPct),i;if(r.length>0){let o=new Set(t.map(l=>String(l.date)));i=r.filter(l=>o.has(String(l.date))).map(l=>l.cumulativeReturnPct)}else i=t.map(()=>0);D&&D.destroy(),D=new Chart(e,{type:"line",data:{labels:n.slice(0,100),datasets:[{label:"Portfolio",data:a.slice(0,100),borderColor:"#4facfe",backgroundColor:"rgba(79, 172, 254, 0.1)",borderWidth:2,pointRadius:2,fill:!0,tension:.1},{label:"Benchmark ("+s.benchmarkSelect.options[s.benchmarkSelect.selectedIndex].text+")",data:i.slice(0,100),borderColor:"#4caf50",borderWidth:2,pointRadius:2,fill:!1,tension:.1}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{position:"top",labels:{color:"#e0e0e0",font:{size:12}}},tooltip:{backgroundColor:"rgba(0,0,0,0.8)",titleColor:"#fff",bodyColor:"#e0e0e0",borderColor:"rgba(255,255,255,0.1)",borderWidth:1,callbacks:{label:o=>o.dataset.label+": "+o.parsed.y.toFixed(2)+"%"}},title:{display:!0,text:"Cumulative Returns Comparison",color:"#fff",font:{size:14}}},scales:{x:{grid:{color:"rgba(255,255,255,0.05)"},ticks:{color:"#888",maxTicksLimit:10}},y:{grid:{color:"rgba(255,255,255,0.05)"},ticks:{color:"#888",callback:o=>o+"%"},border:{color:"rgba(255,255,255,0.1)"}}}}})}async function tt(t){if(!h)return;s.generateInsightsBtn.disabled=!0,s.insightsStatus.style.display="block",s.insightsOutput.textContent="",s.insightsOutput.className="insights-output";let r=!!t;try{let e=await J(b(),t?{apiKey:t}:{},n=>{s.insightsStatus.textContent=n});s.insightsStatus.style.display="none",s.insightsOutput.innerHTML=Dt(e)+(r?"":'<p class="insights-tier-note">Generated with free model. Enter your OpenRouter API key below for deeper analysis.</p>')}catch(e){s.insightsStatus.style.display="none",s.insightsOutput.textContent="Error: "+(e instanceof Error?e.message:String(e)),s.insightsOutput.className="insights-output error"}finally{s.generateInsightsBtn.disabled=!s.apiKey.value.trim()}}async function Lt(){let t=s.apiKey.value.trim();t&&await tt(t)}function Dt(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/\*(.+?)\*/g,"<em>$1</em>").replace(/`(.+?)`/g,"<code>$1</code>").replace(/^- (.+)$/gm,"<li>$1</li>").replace(/((?:<li>.*<\/li>\n?)+)/g,"<ul>$1</ul>").replace(/\n{2,}/g,"<br><br>").replace(/\n/g,"<br>")}function O(t){s.loadingText.textContent=t,s.loadingSection.style.display="flex"}function y(){s.loadingSection.style.display="none"}function R(t){s.loadingSection.style.display="none",s.errorMessage.textContent=t,s.errorSection.style.display="block",s.errorSection.style.visibility="visible",s.errorSection.style.opacity="1"}document.addEventListener("DOMContentLoaded",()=>{ht(),w()});window.PortfolioAnalysis={init:w,parseCSV:C,handleFileUpload:X};
